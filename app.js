...
(function () {
  ...
  function getVolunteerName(id) {
    ...
    var vol = volunteersState.find(function (v) {
      return v.id === id;
    });
  ...
  function renderVolunteerPortal() {
    ...
    if (vol.task) {
      var activeTask = incidentsList.find(function (inc) {
      ...
      if (activeTask) {
        ...
        if (activeTask.status === STATUS.DISPATCHED) {
          actionButtonsHTML =
            ...
            '<button class="btn primary" data-action="acknowledge" data-incident-id="' +
            sanitizeHTML(activeTask.id) +
            '">Acknowledge</button>' +
            "</div>";
        } else if (activeTask.status === STATUS.IN_PROGRESS) {
          actionButtonsHTML =
            '<div class="task-action-btns">' +
            '<button class="btn success" data-action="resolve" data-incident-id="' +
            sanitizeHTML(activeTask.id) +
        ...
        card.innerHTML =
          ...
          activeTask.severity.toLowerCase() +
          '">' +
          sanitizeHTML(activeTask.severity) +
          " Severity</span>" +
          "</div>" +
          '<div class="task-title">' +
          sanitizeHTML(activeTask.type) +
          ": Location " +
          sanitizeHTML(activeTask.location) +
          "</div>" +
          '<p style="font-size: 12px; color: var(--text-secondary); line-height: 1.4;">' +
          sanitizeHTML(activeTask.details) +
          "</p>" +
          '<div class="task-meta">' +
          "<span>📍 " +
          sanitizeHTML(activeTask.location) +
          "</span>" +
          "<span>⏱️ " +
          formatTime(activeTask.reportedAt) +
          "</span>" +
          "</div>" +
          actionButtonsHTML;
  ...
  function renderIncidentLogs() {
    ...
    if (sortedIncidents.length === 0) {
      container.innerHTML =
        '<div style="text-align:center; padding: 20px; color: var(--text-muted); font-size:12px;">No logged incidents. Stadium functioning nominal.</div>';
      return;
    }
    ...
    sortedIncidents.forEach(function (inc) {
      ...
      var assignedVol = volunteersState.find(function (v) {
      ...
      card.innerHTML =
        ...
        '<span class="incident-log-loc">📍 ' +
        sanitizeHTML(inc.location) +
        " • " +
        sanitizeHTML(inc.type) +
        "</span>" +
        '<span class="status-badge ' +
        inc.status.toLowerCase().replace(" ", "") +
        '">' +
        sanitizeHTML(inc.status) +
        "</span>" +
        "</div>" +
        '<div class="incident-log-desc">' +
  ...
  function handleVolunteerAcknowledge(incId) {
    var inc = incidentsList.find(function (i) {
      return i.id === incId;
    });
    if (!inc) return;
  ...
  function handleOrganizerQuery() {
    ...
    setTimeout(function () {
      ...
      var activeIncidents = incidentsList.filter(function (i) {
        return i.status !== STATUS.RESOLVED;
      });
      var activeCount = activeIncidents.length;
      var availVolunteers = volunteersState.filter(function (v) {
        return v.status === STATUS.AVAILABLE;
      });
      ...
      // 2. "Check status of Section 102 spill and list closest available volunteers."
        ...
        if (spillInc2) {
          var assignedCrew2 = volunteersState.find(function (v) {
            return v.id === spillInc2.assignedTo;
          });
          var availableNearby = volunteersState.filter(function (v) {
            return v.status === STATUS.AVAILABLE;
          });

          var crewListStr = availableNearby
            .map(function (v) {
              return (
                sanitizeHTML(v.name) +
                " (Zone: " +
      ...
      // 4. Default search fallback — user query is sanitized
        ...
        var activeIncList = incidentsList
          .filter(function (i) {
            return i.status !== STATUS.RESOLVED;
          })
          .map(function (i) {
